...
class Home extends Component {
	...
	componentDidMount() {
		...
		axios
			.get(
				'https://min-api.cryptocompare.com/data/pricemultifull?fsyms=BTC,ETH,XRP,BCH,EOS,LTC,XLM,IOT,SC,GNT,MTL,PIVX,PART,QTUM,UBQ,LSK,SYS,BNB,QASH,REQ,AMB,BAT,POWR,PAY,OMG,ENJ,KNC,XLM,SALT,BNT,SUB,ARK,ZEC,STRAT,ZRX,ANT,QSP&tsyms=NGN'
			)
			.then(res => {
	...
	handleClick = (e) => {
		...
		axios
			.get(
				'https://min-api.cryptocompare.com/data/pricemultifull?fsyms=' + coinlist +'&tsyms=NGN'
			)
			.then(res => {
				const crypto = res.data;
				this.setState({
